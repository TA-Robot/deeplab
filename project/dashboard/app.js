...
async function main() {
  ...
  if (report.dataset) {
    datasetInfo.textContent = `MNIST (train ${report.dataset.train_size}, val ${report.dataset.val_size}, test ${report.dataset.test_size})`;
  }
  ...
  const guardrail = report.guardrail ?? 0.97;
  if (banner) {
    const warnings = [];
    runKeys.forEach((key) => {
    ...
    Object.entries(report.comparisons || {}).forEach(([pair, cmp]) => {
  ...
  const deltaDatasets = deltaMetrics.map((metric) => {
    const values = ["mlp", "cnn"].map((pair) => {
      const cmp = report.comparisons?.[pair];
  ...
  Object.entries(report.comparisons || {}).forEach(([key, cmp]) => {
  ...
  if (deltaCards) {
    comparisonPairs.forEach((pair) => {
      const cmp = report.comparisons?.[pair.key];
      if (!cmp) return;
    const card = document.createElement("div");
  ...
  if (analysisSections && report.analysis_sections) {
